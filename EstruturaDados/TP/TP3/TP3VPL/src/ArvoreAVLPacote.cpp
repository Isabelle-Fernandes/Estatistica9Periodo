...
void ArvoreAVLPacote::insere(int id, int indice_evento, int tempo) {
    raiz = insereRecursivo(raiz, id, indice_evento, tempo);
}

No* ArvoreAVLPacote::insereRecursivo(No* no, int id, int indice_evento, int tempo) {
    if (no == nullptr) {
        No* novoNo = new No(id, tempo);
        novoNo->indices_eventos.enfileira(indice_evento);
        return novoNo;
    }

    if (id < no->id_pacote) {
        no->esquerda = insereRecursivo(no->esquerda, id, indice_evento, tempo);
    } else if (id > no->id_pacote) {
        no->direita = insereRecursivo(no->direita, id, indice_evento, tempo);
    } else {
