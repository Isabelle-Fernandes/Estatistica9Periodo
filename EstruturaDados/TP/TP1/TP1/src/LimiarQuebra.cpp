...
int LimiarQuebra::DeterminaLimiarQuebra(Vetor &vetor, int tam, double limiarCusto, int tamMinParticao, int seed){
    ...
    while ((diffCusto > limiarCusto) && (numMQS >= 10)){
        ...
        this->ImprimeEstatisticas(iteracao, limQuebrasAntesCalculaFaixa, diffCusto, numMQS);
        iteracao++;
    }
...
void LimiarQuebra::ImprimeEstatisticas(int _iteracao, int _limiteQuebras, double _diffCusto, int _numMQS){
    std::cout << "iter " << _iteracao << std::endl;
    for (int i = 0; i < _numMQS; i+=2){
        std::cout << "qs lq " << this->custoQuebra[i]->quebra << " ";
        std::cout << "cost " << std::fixed << std::setprecision(9) << this->custoQuebra[i]->custo << " ";
        std::cout << "cmp " << this->custoQuebra[i]->comparacoes << " ";
        std::cout << "move " << this->custoQuebra[i]->movimentacoes << " ";
        std::cout << "calls " << this->custoQuebra[i]->chamadas << std::endl;

        std::cout << "in lq " << this->custoQuebra[i+1]->quebra << " ";
        std::cout << "cost " << this->custoQuebra[i+1]->custo << " ";
        std::cout << "cmp " << this->custoQuebra[i+1]->comparacoes << " ";
        std::cout << "move " << this->custoQuebra[i+1]->movimentacoes << " ";
        std::cout << "calls " << this->custoQuebra[i+1]->chamadas << std::endl;
    }
    std::cout << "numlq " << _numMQS/2 << " ";
    std::cout << "limQuebras " << custoQuebra[_limiteQuebras]->quebra << " ";
    std::cout << "lqdiff " << std::fixed << std::setprecision(6) << _diffCusto << std::endl;
    std::cout << std::endl;
}
