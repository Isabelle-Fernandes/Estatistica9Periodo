...
int main(int argc, char* argv[]){
    ...
    for(int i = 0; i < qtdePacote; i++){
        ...
        //ImprimeRota(pacotes[i].rota, pacotes[i].tamanhoRota);
    }

    //pacotes[1].imprimePacote();
...
//////////////   ESCALONADOR EM ACAO!!!  ///////////////////////////
    ...
    while (pacotesEntregues < qtdePacote) {
        ...
        if (evento.tipo == PACOTE) {
            ...
            p->mudarEstado(armazenado, relogioAtual);


            if(p->armazemDestino == armazemDeChegada){
                //std::cout << evento.chavePrioridade << " ";
                p->mudarEstado(entregue, relogioAtual);
            ...
            while(!paraRearmazenar.estahVazia() && contador < capacidadeTransporte) {
                ...
                //std::cout << evento.chavePrioridade << " ";
                p->mudarEstado(alocadoTransporte, tempoFinalOperacao);
            ...
            while(!paraRearmazenar.estahVazia()) {
                Pacote* p = paraRearmazenar.desempilha();
                //std::cout << evento.chavePrioridade << " ";
                p->mudarEstado(rearmazenado, tempoFinalOperacao);
    ...
    // std::cout << "ID;" << "Tempo total;" << "Tempo Transito;"
    //           << "Tempo Armazenado;" << "Tempo Rearmazenado;" << std::endl;

    for (int i = 0; i < qtdePacote; i++) {
        std::cout << capacidadeTransporte << ";" << intervaloTransporte << ";" 
        << qtdeArmazem << ";" << qtdePacote << ";";
        pacotes[i].estatisticas.imprimir(pacotes[i].id, pacotes[i].estatisticas.tempoUltimaMudancaDeEstado);
    }
